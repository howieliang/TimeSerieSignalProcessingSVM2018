...
int d = 2;     //Number of features to feed into the SVM
int lastPredY = -1;
...
void draw() {
  ...
  if (svmTrained) text("Last Prediction:"+lastPredY, 20, 100);
  else text("Current Label: "+label, 20, 100);
  popStyle();
...
void serialEvent(Serial port) {   
  ...
  if (inData.charAt(0) == 'A') {  
    ...
    if (diff>activationThld && sensorHist[0][1]!=0) { //activate when the absolute diff is beyond the activationThld
    ...
    if (b_sampling == true) {
      ...
      if (sampleCnt == windowSize) {
        ...
        if (csvData.getRowCount()<dataNum) { //keep "dataNum" rows of CSV Data
          ...
          newRow.setFloat("m", windowM[0]);
          newRow.setFloat("sd", windowSD[0]);
          newRow.setInt("index", label);
          println("Save Row: ", csvData.getRowCount());
        }
        if (!svmTrained) {
          double[] dataToTrain = {windowM[0], windowSD[0], label}; //Form a dataToTrain with label
          trainData.add(new Data(dataToTrain)); //Add the dataToTrain to the trainingData collection.
          ++tCnt;
          println("train:", label);
        } else {
          double[] dataToTest = {windowM[0], windowSD[0]}; //Form a dataToTest without label Y
